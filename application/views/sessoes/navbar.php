 <?php
   defined('BASEPATH') OR exit('No direct script access allowed');
 
 ?> 

    <!-- //////////////// NAVBAR /////////////////// -->
    <div id="navegar" class="container-fluid fixed-top ">
        <div class="container">
            <nav class="navbar navbar-expand-lg navbar-dark  ">
                <a class="navbar-brand" href="#home">
                    <h2>RA</h2>
                </a>
                <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav"
                    aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
                    <span class="navbar-toggler-icon"></span>
                </button>
                <div class="collapse navbar-collapse pl-5 menu-navegação" id="navbarNav">
                    <ul class="navbar-nav text-success">
                        <li class="nav-item pr-4 pl-4  border-dark ">
                            <a class="nav-link" href="#home">INÍCIO</a>
                        </li>
                        <li class="nav-item pr-4 pl-4">
                            <a class="nav-link" href="#serviços">SERVIÇOS</a>
                        </li>
                        <li class="nav-item pr-4 pl-4">
                            <a class="nav-link" href="#quem_sou">QUEM SOU</a>
                        </li>
                        <li class="nav-item pr-4 pl-4">
                            <a class="nav-link" href="#time-line">TIME-LINE</a>
                        </li>
                        <li class="nav-item pr-4 pl-4">
                            <a class="nav-link" href="#contato">CONTATO</a>
                        </li>
                    </ul>

                </div>
                <div class="d-none d-lg-block">
                    <ul class="navbar-nav text-success pl-5  ">
                        <li class="nav-item   pl-4">
                            <a class="pl-5 nav-link" href="#"><i class="fa fa-facebook-official fa-2x"
                                    aria-hidden="true"></i></a>
                        </li>
                        <li class="nav-item ">
                            <a class="nav-link" href="#"><i class="fa fa-github-square fa-2x"
                                    aria-hidden="true"></i></a>
                        </li>
                        <li class="nav-item ">
                            <a class="nav-link" href="#"><i class="fa fa-linkedin-square fa-2x"
                                    aria-hidden="true"></i></a>
                        </li>
                        <li class="nav-item ">
                            <a class="nav-link" href="#"><i class="fa fa-google-plus-square fa-2x"
                                    aria-hidden="true"></i></a>
                        </li>
                    </ul>
                </div>
            </nav>
        </div>
    </div>